...
void Intro::Render()
{
    ...
    if (isLogoDown)
    {
        ...
        if (sprites->Get(2005))
            sprites->Get(2005)->Draw(0.0f, 60.0f);
        if (sprites->Get(2004))
            sprites->Get(2004)->Draw(0.0f, 0.0f);

        // Vẽ số 3 nhấp nháy đè lên (ID từ 2006 đến 2011)
        int currentNum3SpriteID = 2006 + stateNum3;
        if (sprites->Get(currentNum3SpriteID))
            sprites->Get(currentNum3SpriteID)->Draw(280.0f, 172.0f);

        // Hiện Text Game Start
        if (sprites->Get(2002))
            sprites->Get(2002)->Draw(250.0f, 100.0f);

        // Đặt con trỏ cố định ở giữa text Game Start (Y = 115.0f)
        float cursorX = 215.0f;
        float cursorY = 115.0f;
        if (sprites->Get(2003))
            sprites->Get(2003)->Draw(cursorX, cursorY);
    }
    ...
    else
    {
        if (sprites->Get(2004))
            sprites->Get(2004)->Draw(0.0f, 0.0f);
        if (sprites->Get(2001))
            sprites->Get(2001)->Draw(0.0f, logoY);

        if (!isCurtainUp)
        {
            if (sprites->Get(2000))
                sprites->Get(2000)->Draw(0.0f, curtainY);
        }
    }
}
